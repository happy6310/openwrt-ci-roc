...
# 修改主机名为 Roc
sed -i "s/hostname='.*'/hostname='happyliu'/g" package/base-files/files/bin/config_generate
# 状态页加入签名和时间
...
# OpenAppFilter / Lucky / WeChatPush
...
# ─── 6. PassWall + OpenClash (含 Meta 内核集成) ───────────────────
### PassWall & OpenClash ###

# 移除原 feeds 自带的 PassWall/OpenClash
...
# 把 OpenClash Meta 内核预先推入 core 目录
mkdir -p package/luci-app-openclash/root/etc/openclash/core
curl -L "https://github.com/vernesong/OpenClash/releases/download/latest/clash_meta" \
  -o package/luci-app-openclash/root/etc/openclash/core/clash_meta
